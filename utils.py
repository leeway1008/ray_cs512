...
def filter_partial_tags(tttag,tr) : 
    ...
    #     if tttag[i]['token'].startswith("##") :
    #         tttag[i-1]['token'] += tttag[i]['token'][2:]
    #         ind_remove.append(i)

    for i in range(len(tttag)-1,-1,-1) :
        if tttag[i]['token'].startswith("##") :
            tttag[i-1]['token'] += tttag[i]['token'][2:]
            ind_remove.append(i)
